...
/**
 * Implementation of [BsaTokenProvider] which fetches batches of [ArateaToken] instances of a size
 * configured by [PrivateInferenceConfig.arateaTokenBatchSize].
 */
class AuthenticatingArateaTokenProvider(
  ...
  override suspend fun fetchTokens(
    params: BsaTokenParams<ArateaToken>,
    batchSize: Int,
  ): Result<List<ArateaToken>> = runSuspendCatching {
    require(batchSize in 1..maxBatchSize) { "Invalid batchSize" }
    require(params is ArateaTokenParams) {
      "Only ArateaTokenParms are allowed when fetching ArateaTokens"
    }
    blindSignAuth.createArateaTokens(batchSize, params.challenge)
  }
}
